...
### Requirement: Protocol version strings in visible text
**Reason**: Version strings (`MCP 2026-07-28`) belong in API contracts and specs, not marketing copy. Visible protocol references SHOULD reference the protocol by name only (e.g., "MCP interface").
**Migration**: Remove all hardcoded protocol version strings from component text.

### Requirement: Internal method names in feature descriptions
**Reason**: Method names (`cast_vote`, `tools/call`, `tools/list`, `APPROVE_MEMBER`) are implementation details that confuse non-technical readers and overwhelm technical readers.
**Migration**: Replace with functional descriptions ("vote", "decision submission", "consensus-driven membership").